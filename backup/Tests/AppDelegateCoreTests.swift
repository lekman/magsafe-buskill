...
final class AppDelegateCoreTests: XCTestCase {
  ...
  func testMenuItemStates() {
    ...
    let initialArmItem = menu.items.first {
      $0.title.contains("Arm Protection") && $0.action != nil
    }
  ...
  func testFullWorkflow() {
    ...
    XCTAssertTrue(
      initialStatusItem!.title.contains("Protection Disabled"),
    ...
    DispatchQueue.main.asyncAfter(deadline: .now() + 0.1) {
      ...
      // Recreate menu to get updated state
      let updatedMenu = self.core.createMenu(for: self.mockAppDelegate)
      ...
      XCTAssertTrue(
        updatedStatusItem!.title.contains("Protection Active"),
    ...
    XCTAssertTrue(
      iconName == "shield.fill" || iconName == "shield",
      "Icon should be shield or shield.fill, got: \(iconName)")
    ...
    XCTAssertTrue(
      validStates.contains(core.appController.currentState),
  ...
  func testMenuKeyEquivalents() {
    ...
    XCTAssertTrue(
      keyEquivalents["Arm Protection"] == "a" || keyEquivalents["Disarm Protection"] == "a")
  }
