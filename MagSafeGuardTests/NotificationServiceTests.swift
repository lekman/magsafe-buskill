...
final class NotificationServiceTests: XCTestCase {
    ...
    func testMultipleCriticalAlerts() {
        ...
        for index in 0..<3 {
            testService.showCriticalAlert(
                title: "Critical Alert \(index)",
                message: "Critical message \(index)"
            )
        }
        ...
        for index in 0..<3 {
            XCTAssertEqual(mockDelivery.deliveredNotifications[index].title, "Critical Alert \(index)")
            XCTAssertEqual(mockDelivery.deliveredNotifications[index].message, "Critical message \(index)")
        }
    }
