...
//  MagSafeGuardTests
//
//  Unit tests for FeatureFlags functionality
//
...
final class FeatureFlagsTests: XCTestCase {
    ...
    func testDefaultFlagValues() {
        ...
        for flag in expectedEnabledFlags {
            XCTAssertTrue(FeatureFlags.shared.isEnabled(flag), 
    ...
    func testConvenienceProperties() {
        XCTAssertEqual(FeatureFlags.shared.isPowerMonitoringEnabled, 
                      FeatureFlags.shared.isEnabled(.powerMonitoring))
        XCTAssertEqual(FeatureFlags.shared.isAccessibilityEnabled, 
                      FeatureFlags.shared.isEnabled(.accessibilityManager))
        XCTAssertEqual(FeatureFlags.shared.isNotificationsEnabled, 
                      FeatureFlags.shared.isEnabled(.notificationService))
        XCTAssertEqual(FeatureFlags.shared.isAuthenticationEnabled, 
                      FeatureFlags.shared.isEnabled(.authenticationService))
        XCTAssertEqual(FeatureFlags.shared.isAutoArmEnabled, 
                      FeatureFlags.shared.isEnabled(.autoArmManager))
        XCTAssertEqual(FeatureFlags.shared.isLocationEnabled, 
                      FeatureFlags.shared.isEnabled(.locationManager))
        XCTAssertEqual(FeatureFlags.shared.isNetworkMonitorEnabled, 
                      FeatureFlags.shared.isEnabled(.networkMonitor))
        XCTAssertEqual(FeatureFlags.shared.isSecurityEvidenceEnabled, 
                      FeatureFlags.shared.isEnabled(.securityEvidence))
        XCTAssertEqual(FeatureFlags.shared.isCloudSyncEnabled, 
                      FeatureFlags.shared.isEnabled(.cloudSync))
        XCTAssertEqual(FeatureFlags.shared.isSentryEnabled, 
                      FeatureFlags.shared.isEnabled(.sentryEnabled))
        XCTAssertEqual(FeatureFlags.shared.isPerformanceMetricsEnabled, 
                      FeatureFlags.shared.isEnabled(.performanceMetrics))
        XCTAssertEqual(FeatureFlags.shared.isVerboseLoggingEnabled, 
    ...
    func testConcurrentAccess() {
        ...
        for i in 0..<100 {
            queue.async {
                if i % 2 == 0 {
                    // Write
                    FeatureFlags.shared.setFlag(.verboseLogging, enabled: i % 4 == 0)
                } else {
                    // Read
